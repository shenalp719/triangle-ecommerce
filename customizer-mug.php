...
?>
    ...
    <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // Listen to ALL clicks on the webpage
                document.body.addEventListener('click', async (e) => {
                    ...
                    if (e.target && e.target.id === 'save-product-design') {
                        ...
                        try {
                            ...
                            const canvasData = JSON.stringify({ color: '#ffffff' });

                            console.log("🚀 Taking a screenshot and sending data...");

                            // --- NEW: THE SCREENSHOT CAMERA ---
                            // 1. Force a quick render so the canvas isn't blank
                            if (typeof customizer !== 'undefined' && customizer.renderer) {
                                customizer.renderer.render(customizer.scene, customizer.camera);
                            }
                            // 2. Grab the picture from the canvas!
                            ...
                            const response = await fetch('api/save-design.php', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    preview_image: imageScreenshot // <--- WE ARE NOW SENDING THE PICTURE!
                                })
                            });
